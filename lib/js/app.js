...
NS['cityInfo'] = [
  ...
  { label_ZH: '上海', label_EN: 'Shanghai', code: 'Z9002', major: true },
...
angular.module('weather.directive', [])
  .directive('comSection', function factory() {
    return function postLink(scope, iElement, iAttrs) {
      iElement.foundation('section', { one_up: false })
    }
  })
  .directive('comChosen', function factory() {
    return function postLink(scope, iElement, iAttrs) {
      scope.$watch('$last', function (val) {
        val && iElement.parent().chosen({
  ...
  .directive('comOrbit', function factory() {
    return function postLink(scope, iElement, iAttrs) {
      // Bind Orbit object for later use
      iElement.on('orbit:ready', function (evt, orbit) {
        // May lead to memory leak!
        NS['weatherOrbit'] = orbit
      })

      iElement.foundation('orbit', {
        animation: 'fade',
        timer_speed: 2500,
        pause_on_hover: true,
  ...
  .directive('comImg', function factory() {
    return function postLink(scope, iElement, iAttrs) {
      iElement.on('error', function (evt) {
        this.parentNode.parentNode.removeChild(this.parentNode)
      })
    }
  })

angular.module('weather.filter', [])

var app = angular.module('weather', ['weather.service', 'weather.directive', 'weather.filter'])
  .factory('ns', function () { return NS })

app.controller('AppCtrl', ['$scope', '$location', 'ns', function ($scope, $location, ns) {
  ...
  var updateViews = function (code) {
    ...
    // Move back n frames because of the defer of radar image
    var defer = 4
    datetime.subtract('minutes', interval * (frames + defer))
    datetime.minutes(Math.floor(datetime.minutes() / interval) * interval)
    datetime.seconds(0)
    ...
    for (var i = 0; i < frames; i++) {
      dateFmt1 = datetime.format('YYYY/MM/DD')
      dateFmt2 = datetime.format('YYYYMMDD')
      srcPrefix = '/mocimg/radar/image/' + code + '/QREF/' + dateFmt1 + '/' + code + '.QREF000.' + dateFmt2 + '.'

      $scope.views.push({
        src: srcPrefix + datetime.format('HHmmss') + '.GIF',
        text: 'UTC ' + datetime.format('YYYY-MM-DD HH:mm:ss')
      })
      datetime.add('minutes', interval)
    }
  }

  $scope.citySelect = function (code) {
    ...
    refreshTimer = setTimeout(function () {
      $scope.$apply(function () {
        $scope.citySelect(code)
      })
    }, 10 * 60 * 1000)
  }

  $scope.$watch('cityOption', function (code) {
    $location.path(code)
  })
  ...
  $scope.$watch(function () {
    ...
    var code = path.substr(1)
    if (!code) { $location.path('Z9010') }
    else { $scope.citySelect(code) }
  })
}])
