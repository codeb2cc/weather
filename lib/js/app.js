...
app.controller('AppCtrl', ['$scope', '$timeout', 'ns', function ($scope, $timeout, ns) {
  ...
  $scope.citySelect = function (evt, code) {
    ...
    if (code !== undefined) {
      ...
      $scope.cityCode = $scope.cityOption
    }

    updateViews($scope.cityCode)
    ...
    refreshTimer = setTimeout(function () {
      $scope.$apply(function () {
        $scope.citySelect(undefined, $scope.cityCode)
      })
    }, 10 * 60 * 1000)
  ...
  $timeout(function () {
    $scope.citySelect(null, $scope.cityInfo[0]['code'])
  }, 0)
}])
